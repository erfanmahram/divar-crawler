...
import logzero
from models import create_engine, connection_string, IranAdvertises, PageStatus, StatusException
...
#
logzero.logfile("./logs/iran_ads_logfile.log", maxBytes=1e7, backupCount=10, encoding='utf-8')
...
def crawl_with_selen(url, counter):
    ...
    driver.add_cookie({'name': 'token',
                       'value': 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..wzwwY723hBYF7yJCKh5S5Fi48r1NJhVHhD0ZaH8zz5g'})
    # else:
    #     driver.add_cookie({'name': 'token',
...
def get_images(id, urls):
    ...
    for i, url in enumerate(urls):
        with requests.get(url, stream=True) as req:
            ...
            os.makedirs(os.path.dirname(f"iran_images/{id}/image{i}.jpg"), exist_ok=True)
            with open(f"iran_images/{id}/image{i}.jpg", "wb+") as img:
                for chunk in req.iter_content(chunk_size=8192):
                    img.write(chunk)
        addresses.append(f"iran_images/{id}/image{i}.jpg")
    return addresses
...
def crawl_commercials():
    ...
    with Session(engine) as session:
        urls = session.query(IranAdvertises).filter(IranAdvertises.Status != PageStatus.Finished).order_by(
            func.random()).order_by(IranAdvertises.RetryCount.asc()).limit(5).all()
    logger.info(f"These urls {urls} are going to crawl")
    for url in urls:
        with Session(engine) as session:
            session.query(IranAdvertises).filter(url.Id == IranAdvertises.Id).update(
                {IranAdvertises.RetryCount: url.RetryCount + 1})
            logger.info(f'crawling on {url}')
            _ads = session.query(IranAdvertises).filter(IranAdvertises.Id == url.Id).first()
            try:
                counter *= -1
